...
- Phase 0-4 runtime flow is role-separated. `lpac profile zk-register` talks to the MNO and applet for RegisterAndIssue (BF44/BF45), `lpac profile zk-certinit` talks to the PCA for CertInit (BF46/BF47), `lpac profile zk-order` talks to the MNO for ZKRequest/OrderProfile (BF42/BF43), and `lpac profile zk-download` performs the final SM-DP+ profile download through the applet. `pysim/mno-server.py` owns `FIXED_MNO_PRIVATE_SCALAR`, verifies BF42, calls SM-DP+ ES2+ `downloadOrder` / `confirmOrder`, and returns a BF43 TLV. `pysim/pca-server.py` issues `PCert_U`.
- `zkesim_workflow.sh` intentionally uses a normal prerequisite download through the default ISD-R AID before any ZK phase unless `SKIP_DOWNLOAD=1`. After that it switches `LPAC_CUSTOM_ISD_R_AID` to the ZK applet AID and runs `zk-register` -> `zk-certinit` -> `zk-order` -> `zk-download`; by default the SM-DP+ role runs on port 443, the MNO on 4443, and the PCA on 5443.
...
- Workflow server logs go to `.zkesim-workflow/logs/` by default (`smdpp.log`, `mno.log`, `pca.log`, and per-phase `zk-lpac.log.phase*`). The script refuses to start if the chosen SM-DP+, MNO, or PCA port is already occupied, because stale listeners previously caused false "server running" status.
